                                    ...
                                    bool aAddedByUser )
{
    ...
    {
        if( FieldNamesAreDuplicates( m_viewControlsDataModel->GetUntranslatedFieldName( row ), aFieldName ) )
        {
            return;
        }
    }
...
void DIALOG_FIELDS_TABLE::SetupColumnProperties( int aCol )
{
    ...
    else if( getDataModel()->GetColFieldName( aCol ) == GetDefaultFieldName( FIELD_T::DATASHEET, UNTRANSLATED ) )
    {
...
void DIALOG_FIELDS_TABLE::SetupAllColumnProperties()
{
    ...
    {
        int col = getDataModel()->GetFieldNameCol( m_viewControlsDataModel->GetUntranslatedFieldName( i ) );
...
void DIALOG_FIELDS_TABLE::OnRenameField( wxCommandEvent& aEvent )
{
    ...
    m_viewControlsDataModel->SetUntranslatedFieldName( row, newFieldName );
...
void DIALOG_FIELDS_TABLE::doApplyBomPreset( const BOM_PRESET& aPreset )
{
    ...
    for( int i = 0; i < getDataModel()->GetColsCount(); i++ )
    {
        ...
        {
            if( m_viewControlsDataModel->GetUntranslatedFieldName( j ) == fieldName )
            {
                found = true;
                break;
            }
    ...
    for( int i = 0; i < m_viewControlsDataModel->GetNumberRows(); i++ )
    {
        const wxString& fieldName( m_viewControlsDataModel->GetUntranslatedFieldName( i ) );
        int             col = getDataModel()->GetFieldNameCol( fieldName );
