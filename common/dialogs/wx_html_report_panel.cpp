...
WX_HTML_REPORT_PANEL::WX_HTML_REPORT_PANEL(wxWindow *parent, wxWindowID id,
        const wxPoint &pos, const wxSize &size, long style) :
        WX_HTML_REPORT_PANEL_BASE(parent, id, pos, size, style), m_reporter(
                this), m_severities(-1), m_lazyUpdate(false) {
    syncCheckboxes();
...
void WX_HTML_REPORT_PANEL::Report(const wxString &aText,
        REPORTER::SEVERITY aSeverity, REPORTER::LOCATION aLocation) {
    REPORT_LINE line;
    line.message = aText;
...
static wxBitmap makeBadge(REPORTER::SEVERITY aStyle, int aCount,
        wxWindow *aWindow) {
    ...
    brush.SetColour(
            aWindow->GetParent()->GetBackgroundColour().MakeDisabled(210));
    ...
    badgeDC.DrawCircle(size.x / 2 - 1, size.y / 2,
            (std::max(size.x, size.y) / 2) - 1);

    wxFont font(fontSize, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL,
            wxFONTWEIGHT_BOLD);
    ...
    badgeDC.DrawText(text, size.x / 2 - textExtent.x / 2 - 1,
            size.y / 2 - textExtent.y / 2);
...
void WX_HTML_REPORT_PANEL::updateBadges() {
    ...
    m_errorsBadge->SetBitmap(
            makeBadge(REPORTER::RPT_ERROR, count, m_errorsBadge));

    count = Count(REPORTER::RPT_WARNING);
    m_warningsBadge->SetBitmap(
            makeBadge(REPORTER::RPT_WARNING, count, m_warningsBadge));
}
...
wxString WX_HTML_REPORT_PANEL::addHeader(const wxString &aBody) {
    ...
    return wxString::Format(
            wxT("<html><body bgcolor='%s' text='%s'>%s</body></html>"),
            bgcolor.GetAsString(wxC2S_HTML_SYNTAX),
            fgcolor.GetAsString(wxC2S_HTML_SYNTAX), aBody);
}
...
wxString WX_HTML_REPORT_PANEL::generateHtml(const REPORT_LINE &aLine) {
    ...
    case REPORTER::RPT_ERROR:
        retv = "<font color=\"red\" size=2><b>" + _("Error: ")
    ...
    case REPORTER::RPT_WARNING:
        ...
        break;
    case REPORTER::RPT_INFO:
        retv = "<font color=\"dark gray\" size=2><b>" + _("Info: ") + "</b>"
                + aLine.message + "</font><br>";
        break;
    case REPORTER::RPT_ACTION:
        retv = "<font color=\"dark green\" size=2>" + aLine.message
                + "</font><br>";
        break;
    default:
    ...
    if (event.GetId() == wxID_COPY) {
        if ( wxTheClipboard->Open()) {
            ...
            wxTheClipboard->SetData(
                    new wxTextDataObject(m_htmlView->SelectionToText()));
...
void WX_HTML_REPORT_PANEL::onBtnSaveToFile(wxCommandEvent &event) {
    ...
    wxFileDialog dlg(this, _("Save Report to File"), fn.GetPath(),
            fn.GetFullName(), TextFileWildcard(),
