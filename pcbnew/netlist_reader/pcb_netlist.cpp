...
{
    const COMPONENT_NET* fallback = nullptr;

    for( const COMPONENT_NET& net : m_nets )
    {
        if( net.GetPinName() != aPinName )
        {
            if( !net.GetPinName().StartsWith( wxS( "[" ) ) )
                continue;

            const std::vector<wxString> expanded = ExpandStackedPinNotation( net.GetPinName() );

            if( std::find( expanded.begin(), expanded.end(), aPinName ) == expanded.end() )
                continue;
        }
        ...
        // Shared-unit pins can export both an explicit net and automatic unconnected nets.
        if( !net.GetNetName().IsEmpty() && !IsAutoGeneratedNetName( net.GetNetName() ) )
            return net;
    }

    return fallback ? *fallback : m_emptyNet;
}
