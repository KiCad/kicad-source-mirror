...
void PCB_EDIT_FRAME::ReCreateMenuBar()
{
    ...
    auto modifiedDocumentCondition = [this]( const SELECTION& sel ) { return IsContentModified(); };
    ...
    if( Kiface().IsSingle() ) // not when under a project mgr
    {
        ...
        fileMenu->AddItem( ID_IMPORT_NON_KICAD_BOARD, _( "Import Non-KiCad Board File..." ),
                _( "Import board file from other applications" ), import_brd_file_xpm,
    ...
    fileMenu->AddItem( ID_MENU_RECOVER_BOARD_AUTOSAVE, _( "Resc&ue" ),
            _( "Clear board and get last rescue file automatically saved by Pcbnew" ), rescue_xpm,
            SELECTION_CONDITIONS::ShowAlways );

    fileMenu->AddItem( ID_MENU_READ_BOARD_BACKUP_FILE, _( "Revert to Last Backup" ),
    ...
    submenuExport->Add(
            _( "SVG..." ), _( "Export SVG board representation" ), ID_GEN_PLOT_SVG, plot_svg_xpm );
    ...
    submenuExport->Add( _( "Hyperlynx..." ), "", ID_GEN_EXPORT_FILE_HYPERLYNX, export_step_xpm );
    ...
    auto gridShownCondition     = [this]( const SELECTION& aSel ) { return IsGridVisible(); };
    auto polarCoordsCondition   = [this]( const SELECTION& aSel ) { return GetShowPolarCoords(); };
    ...
    drawingModeSubMenu->AddCheckItem(
            PCB_ACTIONS::moduleEdgeOutlines, sketchModuleEdgeOutlinesCondition );
    drawingModeSubMenu->AddCheckItem(
            PCB_ACTIONS::moduleTextOutlines, sketchModuleTextOutlinesCondition );
