...
doc: |
  PADS PowerPCB binary board file. Versions 0x2021, 0x2022, 0x2024, 0x2025,
  0x2026, 0x2027. Section payloads follow the directory contiguously in directory
  order; each start is the running sum of prior sections' total_bytes LESS the
  section-3 over-declaration (see `dir_entry`). The directory stores no explicit
  per-section offset, and the entry count is stored rather than implied by the
  version: v0x2021, v0x2022 and v0x2024 have 73 entries, v0x2025 onward 74.

  SECTION LOCATION SUMMARY (measured against the 163-file corpus):

      entry_count    = directory[1].count - 1
      payload_offset = data_offset - (entry_count*16 + 48)   for sections after 3

      sections 3..22    base = payload_offset          written == declared
      section 14        base = payload_offset + 44     decal names, 165/165
      section 23        base = payload_offset          nets
      section 24        base = payload_offset + 76     connections, 46/46;
                                                       declared count is LIVE
      sections 26..55   base = payload_offset          written == declared
      section 56        live 16-byte records, then the string index
      section 57        the string pool; total_bytes IS its true byte length
      sections 58..     declared sizes resume
      sections 69..72   base = payload_offset          written == declared
