...
Because a footprint's zones are built after step 3, the footprint's
Flip()/Rotate() has already been applied and will not touch them.
Build footprint zone polygons (and related fills) straight from the
board-absolute Allegro geometry, the same as graphics and text.
