...
## 2026-01-26 through 2026-02-08

Major development push bringing the importer from basic parsing to a comprehensive import:

* Coordinate system and geometry fixes:
  * Fixed coordinate scaling (25400.0/divisor nm, Y negated)
  * Extracted arc direction from m_SubType bit 6 (0x40 = clockwise)
  * Fixed via drill extraction for V172+ (m_DrillArr[4] width, [7] height)
  * Fixed pad position/orientation to use FP-relative coordinates (SetFPRelativePosition)
  * Fixed footprint flip ordering (fp->Flip() must be called AFTER adding all children)

* Pad and padstack improvements:
  * Added all 18 pad shape types from OpenAllegroParser enum
  * Shape symbol pads (type 0x16) resolved via m_StrPtr -> 0x28 polygon block
  * Slot hole detection for routed and punched oblong drills
  * Version-dependent drill extraction (pre-V172 m_Drill vs V172+ m_DrillArr)

* Zone and net improvements:
  * Zone outlines from BOUNDARY class (0x15) shapes on m_LL_Shapes linked list
  * Zone net resolution via BOUNDARY.Ptr7 -> 0x2C TABLE -> 0x37 array -> 0x1B NET chain
  * Fixed SetLayer() must be called BEFORE SetNetCode() on zones
  * Multi-layer zone merging for copper pours spanning multiple layers

* Layer mapping:
  * Full LAYER_MAPPABLE_PLUGIN integration for user-customizable layer mapping
  * Board outline subclass codes DFMT_OUTLINE (0xFD) and BGEOM_OUTLINE (0xEA)
  * AUTOSILK_BOTTOM mapped to B_SilkS, AUTOSILK_TOP to F_SilkS
  * Fixed DISPLAY_TOP/BOTTOM layer mapping for text classes (copy-paste bug)
  * Custom layers recorded in m_MappedOptionalLayers to prevent duplicates

* Design rules and netclasses:
  * Physical constraint sets (0x1D blocks) imported as KiCad netclasses
  * Constraint set name resolution via three-tier fallback (string table, 0x03 FIELD, generic CS_N)
  * Per-net trace widths imported as individual netclasses
  * Differential pairs (2-net groups) and match groups (>2 nets) imported as netclasses
  * NET field 0x1a0 links nets to constraint sets (integer key or direct string)
  * Uses Eagle pattern for netclass assignment (both pattern + direct) for save/reload persistence

* Text and field conversion:
  * REF_DES on SILKSCREEN -> built-in REFERENCE field
  * REF_DES on ASSEMBLY -> user field "Reference" with ${REFERENCE} variable
  * COMPONENT_VALUE on ASSEMBLY -> built-in VALUE field
  * DEVICE_TYPE, TOLERANCE, USER_PART_NUMBER -> user fields with class-aware visibility
  * Text rotation (millidegrees), stroke width (height/8), mirroring, alignment

* Courtyard and fabrication:
  * Assembly outlines (PGEOM_ASSEMBLY) duplicated to courtyard layers with zero width
  * PLACE_BOUND polygons (0x28) map to courtyard
  * Tech layer canonicalization for bottom-side footprints (layer-only, no position change)
  * Zero-width lines get GetLineThickness(layer) fallback

* Reverse engineering:
  * 0x1E blocks identified as IBIS/simulation netlists (not imported)
  * 0x1F blocks identified as per-padstack dimension records (not imported)
  * 0x27 block identified as serialized Constraint Manager DB (not imported)
  * LL_WALKER fixed for 0x1D/0x1E/0x1F traversal (GetPrimaryNext with m_Next)

* Test suite:
  * 12 test boards in qa/data/pcbnew/plugins/allegro/ (PreAmp is 0 bytes, excluded)
  * 11 AllegroImport tests covering geometry, zones, pads, vias, layers, text, fields
  * 41 AllegroComprehensive tests for cross-validation against .alg ASCII references
  * Tests for SMD pad layer consistency and UI import path
  * 52 tests total, all passing

* Code cleanup:
  * Removed extract and pycli utilities (development-only tools)
  * Removed FORMAT_INFO.md, allegro_brd.ksy, CLAUDE.md, dump scripts
  * Renamed hex-only struct names to descriptive names throughout
  * Updated comments in allegro_pcb_structs.h with current knowledge
  * Moved WORKLOG.md from utils/ to main allegro/ directory

## 2026-02-09

* Zone fill polygon import:
  * 0x28 shapes on net assignment chains (0x1B -> 0x04 -> 0x05) are computed copper fills
  * Collected during createTracks() into m_zoneFillShapes with net code and layer
  * New applyZoneFills() matches fills to zones by net code + layer + bounding box overlap
  * Uses CADSTAR pattern: SetFilledPolysList, SetIsFilled, SetNeedRefill(false)
  * ClearArcs() required on fill outlines before Fracture() (boolean ops don't support arcs)
  * Fallback net resolution: when resolveShapeNet() returns UNCONNECTED, search fills
    for matching layer/bbox to determine the zone's net

* Drill slot orientation fix:
  * Allegro stores slot dimensions as (primary, secondary) not (X, Y)
  * OBLONG_Y pads were getting horizontal slots instead of vertical
  * Fix compares first copper pad aspect ratio with drill aspect ratio, swaps if mismatched
  * Safe for round drills (equal W/H skips the check)

* Diff pair gap import from constraint set DataB records:
  * f[7] in the 56-byte DataB record holds the diff pair gap for both V172+ and pre-V172
  * Added diffPairGap to CS_DEF struct in applyConstraintSets()
  * Sets both SetDiffPairGap() and SetDiffPairWidth() (width = track width for the pair)
  * Match group netclasses now inherit diff pair gap and width from the underlying
    constraint set netclass (previously only inherited clearance and track width)

* Test suite expansion:
  * 56 tests total (11 AllegroImport + 45 AllegroComprehensive)
  * New tests: ConstraintSetDiffPairGap (BB Black V172+), ConstraintSetDiffPairGapPreV172 (VCU118)
  * Zone fill and drill slot orientation verified against .alg reference data
