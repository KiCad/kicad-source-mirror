...
/**
 * Board-wide diagnostics for the constraint overlay and info bar: each constrained shape's state
 * plus the conflicting/redundant constraints and the total remaining free degrees of freedom.
 */
...
/**
 * The bilingual layer between KiCad board geometry and the planegcs solver (issue #2329).
 *
 * Build() translates a cluster of #PCB_SHAPEs and the #PCB_CONSTRAINTs among them into a
 * planegcs system, scaled into a conditioned (millimetre) frame because raw IU (nanometres)
 * squared in the residuals is badly conditioned.  Solve() runs the solver (optionally pinning
 * a dragged anchor to a cursor position), Apply() writes the solution back to the shapes in IU,
 * and Diagnose() reports the constraint state.
 *
 * The adapter owns a pointer-stable std::deque<double> backing store for every value the solver
 * touches -- point coordinates AND driving constants (lengths, pin targets).  planegcs holds raw
 * double* into this deque, so it must never reallocate, erase, or reorder while the GCS::System is
 * alive.  std::deque guarantees element-pointer stability across push_back, the only growth used.
 */
...
public:
     ...
     *
     * The pin is a temporary (negatively tagged) constraint, so it yields to the real
     * constraints: an over-constrained drag leaves the geometry where the rules allow.
     *
     ...
     */
    std::vector<PCB_SHAPE*> Apply( const std::function<void( PCB_SHAPE* )>& aBeforeWrite = {} );
 ...
 * @param aBeforeModify if set, invoked with each neighbor shape immediately before its geometry
 *                  is changed (the dragged shape is excluded -- the caller stages it itself), so
 *                  the caller can BOARD_COMMIT::Modify it before the change for a clean undo.
 ...
 * @param aBeforeModify if set, invoked with each moved shape just before it changes.
 */
