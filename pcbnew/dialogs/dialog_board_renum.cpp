...
DIALOG_BOARD_RENUM::DIALOG_BOARD_RENUM(PCB_EDIT_FRAME *aParentFrame) :
    ...
    s_savedDialogParameters.RemoveFrontPrefix &= (0
            != s_savedDialogParameters.FrontPrefix.size()); //Don't remove if nothing to remove
    s_savedDialogParameters.RemoveBackPrefix &= (0
            != s_savedDialogParameters.BackPrefix.size());
    ...
    if (0 == s_savedDialogParameters.BackStartRefDes)
        ...
        m_BackRefDesStart->ChangeValue(
                std::to_string(s_savedDialogParameters.BackStartRefDes));

    m_BackPrefix->ChangeValue(s_savedDialogParameters.BackPrefix);
    m_RemoveBackPrefix->SetValue(
            0 == s_savedDialogParameters.RemoveBackPrefix ? false : true);
    m_SortGrid->ChangeValue(std::to_string(s_savedDialogParameters.SortGrid));
    m_WriteChangeFile->SetValue(
            0 == s_savedDialogParameters.WriteChangeFile ? false : true);
    m_WriteLogFile->SetValue(
            0 == s_savedDialogParameters.WriteLogFile ? false : true);
    m_SortOnModules->SetValue(
            s_savedDialogParameters.SortOnModules ? true : false);
    m_SortOnRefDes->SetValue(
            !s_savedDialogParameters.SortOnModules ? true : false);
    ...
    for (auto mod : m_modules) {                    // Create a netlist
        ...
        if (UpdateRefDes == newrefdes->Action)       //Ignore blanks
            netlist.AddComponent(                   //add to the netlist
                    new COMPONENT(mod->GetFPID(), newrefdes->NewRefDes,
...
std::string DIALOG_BOARD_RENUM::CoordTowxString(int aX, int aY) {
    EDA_UNITS units =
            s_savedDialogParameters.RenumDialog->m_frame->GetUserUnits();
...
//
void DIALOG_BOARD_RENUM::WriteRenumFile(const char *aFileType,
        wxString &aBuffer) {
    wxFileName filename =
...
void DIALOG_BOARD_RENUM::LogRefDesTypes(void) {
    int i = 1;
    wxString message = _("\n\n\nThere are ")
            + std::to_string(this->m_RefDesTypes.size());
...
void DIALOG_BOARD_RENUM::LogModules(wxString &aMessage,
        std::vector<RefDesInfo> &aModules) {

    int i = 1;

    wxString tmps, message = aMessage + _("\n*********** Sort on ")
            + (std::string) (
    ...
    for (auto mod : aModules) {
        tmps.Printf(_("\n%d %s Time Stamp: %X, X, Y, %s, Rounded X, Y, %s"),
                i++, mod.RefDesString, mod.TimeStamp,
