...
void DIALOG_SHAPE_PROPERTIES::createLineEndingControls( PCB_BASE_EDIT_FRAME* aParent )
{
    ...
    m_endingsSizer->Add( m_endingsHelpLabel, 0, wxLEFT | wxRIGHT | wxTOP | wxBOTTOM, 5 );
...
bool DIALOG_SHAPE_PROPERTIES::TransferDataToWindow()
{
    ...
    if( !m_item->IsClosed() )
    {
        ...
        m_startThickness->SetValue( m_item->GetStartEndingStrokeWidth() );
        m_endLength->SetValue( m_item->GetEndEndingLength() );
        m_endWidth->SetValue( m_item->GetEndEndingWidth() );
        m_endThickness->SetValue( m_item->GetEndEndingStrokeWidth() );
    }
    ...
    *m_item = m_workingCopy;
    ...
    if( !m_item->IsClosed() )
    {
        ...
        if( !m_startThickness->IsIndeterminate() )
            m_item->SetStartEndingStrokeWidth( std::max( 0, m_startThickness->GetIntValue() ) );
        ...
        if( !m_endThickness->IsIndeterminate() )
            m_item->SetEndEndingStrokeWidth( std::max( 0, m_endThickness->GetIntValue() ) );
    }
