...
BEGIN_EVENT_TABLE( PCB_EDIT_FRAME, PCB_BASE_FRAME ) EVT_SOCKET( ID_EDA_SOCKET_EVENT_SERV, PCB_EDIT_FRAME::OnSockRequestServer )
...
PCB_EDIT_FRAME::PCB_EDIT_FRAME(KIWAY *aKiway, wxWindow *aParent) :
    ...
    GetScreen()->AddGrid(m_UserGridSize, EDA_UNITS::UNSCALED,
            ID_POPUP_GRID_USER);
    ...
    m_auimgr.AddPane(m_auxiliaryToolBar,
            EDA_PANE().HToolbar().Name("AuxToolbar").Top().Layer(4));
    ...
    m_auimgr.AddPane(m_Layers,
            EDA_PANE().Palette().Name("LayersManager").Right().Layer(3).Caption(
                    _("Layers Manager")).PaneBorder(false).MinSize(80, -1).BestSize(
                    m_Layers->GetBestSize()));
    ...
    if (m_firstRunDialogSetting < 1) {
        if (m_canvasType != EDA_DRAW_PANEL_GAL::GAL_TYPE_OPENGL) {
            wxString msg =
                    _(
                            "KiCad can use your graphics card to give you a smoother "
            ...
            if (dlg.ShowModal() == wxID_YES) {
                ...
                if (GetCanvas()->GetBackend()
                        == EDA_DRAW_PANEL_GAL::GAL_TYPE_NONE)
                    GetToolManager()->RunAction(ACTIONS::standardGraphics,
                            true);
            } else {
    ...
    GetCanvas()->GetView()->SetScale(
            GetZoomLevelCoeff() / GetScreen()->GetZoom());
    ActivateGalCanvas();
...
void PCB_EDIT_FRAME::SetPageSettings(const PAGE_INFO &aPageSettings) {
    ...
    worksheet = new KIGFX::WS_PROXY_VIEW_ITEM(IU_PER_MILS,
            &m_Pcb->GetPageSettings(), &m_Pcb->GetTitleBlock());
...
bool PCB_EDIT_FRAME::IsContentModified() {
    return GetScreen() && GetScreen()->IsModify() && GetBoard()
            && !GetBoard()->IsEmpty();
}
...
void PCB_EDIT_FRAME::OnCloseWindow(wxCloseEvent &aEvent) {
    ...
    DIALOG_DRC_CONTROL *open_dlg =
            static_cast<DIALOG_DRC_CONTROL*>(wxWindow::FindWindowByName(
    ...
    if (IsContentModified()) {
        ...
        if (!HandleUnsavedChanges(this,
                wxString::Format(msg, fileName.GetFullName()), [&]() -> bool {
    ...
    if (fn.FileExists() && !wxRemoveFile(fn.GetFullPath())) {
        wxString msg = wxString::Format(
                _("The auto save file \"%s\" could not be removed!"),
                fn.GetFullPath());
...
void PCB_EDIT_FRAME::ActivateGalCanvas() {
    ...
    GetCanvas()->GetView()->GetPainter()->GetSettings()->ImportLegacyColors(
            &cds);
    GetCanvas()->Refresh();
}
...
void PCB_EDIT_FRAME::SetVisibleAlls() {
    ...
    for (GAL_LAYER_ID ii = GAL_LAYER_ID_START; ii < GAL_LAYER_ID_BITMASK_END;
            ++ii)
    ...
    if (fileName.IsOk() && fileName.FileExists())
        fileinfo =
                fileName.IsFileWritable() ?
                        wxString(wxEmptyString) : _(" [Read Only]");
    else
        fileinfo = _(" [Unsaved]");

    SetTitle(
            wxString::Format( _( "Pcbnew" ) + wxT(" \u2014 %s%s"),
...
bool PCB_EDIT_FRAME::TestStandalone(void) {
    if (Kiface().IsSingle()) {
        DisplayErrorMessage(this,
                _(
                        "Cannot update the PCB because Pcbnew is opened in stand-alone "
...
bool PCB_EDIT_FRAME::FetchNetlistFromSchematic(NETLIST &aNetlist,
    ...
    try {
        auto lineReader = new STRING_LINE_READER(payload,
                _("Eeschema netlist"));
...
void PCB_EDIT_FRAME::RunEeschema() {
    ...
    if (Kiface().IsSingle()) {
        wxString filename = wxT(
                "\"") + schfn.GetFullPath( wxPATH_NATIVE ) + wxT( "\"" );
                               ...
                               // but no project loaded.
        {
            frame->OpenProjectFiles(
                    std::vector<wxString>(1, schfn.GetFullPath()));
            frame->Show(true);
        }
...
void PCB_EDIT_FRAME::InstallFootprintPropertiesDialog(MODULE *Module) {
    ...
    DIALOG_FOOTPRINT_BOARD_EDITOR *dlg = new DIALOG_FOOTPRINT_BOARD_EDITOR(this,
            Module);
    ...
    if (retvalue == DIALOG_FOOTPRINT_BOARD_EDITOR::PRM_EDITOR_EDIT_OK) {
        ...
        auto editor = (FOOTPRINT_EDIT_FRAME*) Kiway().Player(
                FRAME_FOOTPRINT_EDITOR, true);
        ...
        editor->Raise();        // Iconize( false );
            ...
            == DIALOG_FOOTPRINT_BOARD_EDITOR::PRM_EDITOR_EDIT_LIBRARY_FOOTPRINT) {
        auto editor = (FOOTPRINT_EDIT_FRAME*) Kiway().Player(
                FRAME_FOOTPRINT_EDITOR, true);
        ...
        editor->Raise();        // Iconize( false );
            ...
            == DIALOG_FOOTPRINT_BOARD_EDITOR::PRM_EDITOR_WANT_UPDATE_FP) {
        InstallExchangeModuleFrame(Module, true, true);
    }

    else if (retvalue
            == DIALOG_FOOTPRINT_BOARD_EDITOR::PRM_EDITOR_WANT_EXCHANGE_FP) {
...
void PCB_EDIT_FRAME::OnExportHyperlynx(wxCommandEvent &event) {
    ...
    wxFileDialog dlg(this, _("Export Hyperlynx Layout"), fn.GetPath(),
            fn.GetFullName(), wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
