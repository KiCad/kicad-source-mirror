...
static void nsvg__parseContent( char* s, void ( *contentCb )( void* ud, const char* s, bool cdata ), void* ud )
{
    if( !*s )
        return;

    if( contentCb )
        ( *contentCb )( ud, s, false );
}
...
int nsvg__parseXML( char* input, void ( *startelCb )( void* ud, const char* el, const char** attr ),
                    ...
                    void ( *contentCb )( void* ud, const char* s, bool cdata ), void* ud )
{
    char* s = input;
    char* mark = s;
...
static NSVGparser* nsvg__createParser( void )
{
    ...
    p->attr[0].fontSize = 16;
    strcpy( p->attr[0].fontFamily, "sans-serif" );
    p->attr[0].fontWeight = FC_WEIGHT_REGULAR;
...
static int nsvg__parseAttr( NSVGparser* p, const char* name, const char* value )
{
    ...
    {
        strncpy( attr->fontFamily, value, sizeof( attr->fontFamily ) - 1 );
        attr->fontFamily[sizeof( attr->fontFamily ) - 1] = 0;
    }
    else if( strcmp( name, "font-weight" ) == 0 )
    {
        int weight = strcmp( value, "bold" ) == 0 ? 700 : atoi( value );
        attr->fontWeight = FcWeightFromOpenType( weight > 0 ? weight : 400 );
    }
    else if( strcmp( name, "font-style" ) == 0 )
    {
        attr->fontItalic = strcmp( value, "italic" ) == 0 || strcmp( value, "oblique" ) == 0;
    }
    else if( strcmp( name, "text-anchor" ) == 0 )
    {
        attr->textAnchor = strcmp( value, "middle" ) == 0 ? 1 : strcmp( value, "end" ) == 0 ? 2 : 0;
    }
    else if( strcmp( name, "xml:space" ) == 0 || strcmp( name, "white-space" ) == 0 )
    {
        attr->preserveSpace = strcmp( value, "preserve" ) == 0 || strcmp( value, "pre" ) == 0;
    }
    else if( strcmp( name, "font-size" ) == 0 )
    {
        attr->fontSize = nsvg__parseCoordinate( p, value, 0.0f, attr->fontSize );
    }
...
static int nsvg__parseNameValue( NSVGparser* p, const char* start, const char* end )
{
    ...
    // Keep the name and value within their half-open ranges.
    while( str > start && nsvg__isspace( str[-1] ) )
        --str;
...
static void nsvg__parseStyle( NSVGparser* p, const char* str )
{
    ...
    while( *str )
    {
        ...
        // Right Trim
        while( end > start && nsvg__isspace( end[-1] ) )
            --end;
...
static void nsvg__parseAttribs( NSVGparser* p, const char** attr )
{
    for( int pass = 0; pass < 3; ++pass )
    {
        ...
        {
            int priority = strcmp( attr[i], "style" ) == 0 ? 2 : strcmp( attr[i], "class" ) == 0 ? 1 : 0;
            if( priority == pass )
...
static void nsvg__parseRect( NSVGparser* p, const char** attr )
{
    ...
    if( w != 0.0f && h != 0.0f )
    {
        ...
        else
        {
            ...
            nsvg__cubicBezTo( p, x + w - rx * ( 1 - NSVG_KAPPA90 ), y, x + w, y + ry * ( 1 - NSVG_KAPPA90 ), x + w,
                              y + ry );
            nsvg__lineTo( p, x + w, y + h - ry );
            nsvg__cubicBezTo( p, x + w, y + h - ry * ( 1 - NSVG_KAPPA90 ), x + w - rx * ( 1 - NSVG_KAPPA90 ), y + h,
                              x + w - rx, y + h );
            nsvg__lineTo( p, x + rx, y + h );
            nsvg__cubicBezTo( p, x + rx * ( 1 - NSVG_KAPPA90 ), y + h, x, y + h - ry * ( 1 - NSVG_KAPPA90 ), x,
                              y + h - ry );
...
static void nsvg__startElement( void* ud, const char* el, const char** attr )
{
    ...
    if( strcmp( el, "text" ) == 0 || strcmp( el, "tspan" ) == 0 )
    {
        nsvg__startText( p, el, attr );
    }
...
static void nsvg__content( void* ud, const char* s, bool cdata )
{
    NSVGparser* p = (NSVGparser*) ud;

    if( p->defsFlag && !p->styleFlag )
        return;

    if( !p->styleFlag )
    {
        nsvg__textContent( p, s, cdata );
        return;
    }
...
{
    return nsvgParseWithFontConfig( input, units, dpi, nullptr );
}

NSVGimage* nsvgParseWithFontConfig( char* input, const char* units, float dpi, FcConfig* fontConfig )
{
    std::unique_ptr<NSVGparser, decltype( &nsvg__deleteParser )> parser( nsvg__createParser(), nsvg__deleteParser );
    NSVGparser*                                                  p = parser.get();
    NSVGimage*                                                   ret = 0;
