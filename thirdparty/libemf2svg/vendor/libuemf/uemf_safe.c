...
// KiCad checks sizes without narrowing them to signed int.
#undef IS_MEM_UNSAFE
#define IS_MEM_UNSAFE(A, B, C) ((uintptr_t)(A) > (uintptr_t)(C) || \
                                (uint64_t)(B) > (uint64_t)((uintptr_t)(C) - (uintptr_t)(A)))
...
int extlogpen_safe(
   ...
   uint64_t count=elp->elpNumEntries;
...
int emrtext_safe(
   ...
   if(IS_MEM_UNSAFE(pemt, sizeof(U_EMRTEXT), blimit))return(0);
   size_t off = sizeof(U_EMRTEXT);
   if(!(pemt->fOptions & U_ETO_NO_RECT))off += sizeof(U_RECTL);
   if(IS_MEM_UNSAFE(pemt, off + sizeof(uint32_t), blimit))return(0);
   uint32_t offDx;
   memcpy(&offDx, (const char *)pemt + off, sizeof(offDx));
   size_t length = (size_t)(blimit - record);
   size_t stride = (pemt->fOptions & U_ETO_PDY) ? 8 : 4;
   size_t charSize = U_EMRTYPE(record) == U_EMR_EXTTEXTOUTA &&
                     !(pemt->fOptions & U_ETO_GLYPH_INDEX) ? 1 : 2;
   if(pemt->offString > length || pemt->nChars > (length - pemt->offString) / charSize)return(0);
   if(offDx && (offDx > length || pemt->nChars > (length - offDx) / stride))return(0);
   return(1);
}
...
int rgndata_safe(
   ...
   if(cbRgnData < (int)sizeof(U_RGNDATAHEADER))return(0);
   uint64_t count = rd->rdh.nCount;
   if(count > (cbRgnData - sizeof(U_RGNDATAHEADER)) / sizeof(U_RECTL))return(0);
   return(1);
}
...
int bitmapinfo_safe(
   ...
   if(IS_MEM_UNSAFE(Bmi, sizeof(U_BITMAPINFOHEADER), blimit))return(0);
   U_BITMAPINFOHEADER header;
   memcpy(&header, Bmi, sizeof(header));
   if(header.biSize < sizeof(header) || IS_MEM_UNSAFE(Bmi, header.biSize, blimit))return(0);
   int64_t height = header.biHeight;
   if(height < 0)height = -height;
   // The decoder uses signed int offsets for its RGBA output.
   if(header.biWidth <= 0 || !height || (uint64_t)header.biWidth * height > INT_MAX / 4)return(0);
   int ClrUsed = get_real_color_count(Bmi);
   if(ClrUsed < 0 || (ClrUsed && IS_MEM_UNSAFE(Bmi, header.biSize + (uint64_t)ClrUsed * sizeof(U_RGBQUAD), blimit)))return(0);
   return(1);
}
...
int DIB_safe(
   ...
   if(!cbBmi)return(cbBits == 0);
   if(offBmi % 4 || IS_MEM_UNSAFE(record, (uint64_t)offBmi + cbBmi, blimit))return(0);
   if(!bitmapinfo_safe(record + offBmi, record + offBmi + cbBmi))return(0);
   if(IS_MEM_UNSAFE(record, (uint64_t)offBits + cbBits, blimit))return(0);
   (void)iUsage;
   {
       ...
       // RLE previews use the WMF fallback instead of the legacy decompressor.
       if(dibparams == U_BI_RLE4 || dibparams == U_BI_RLE8)return(0);
       ...
       if(dibparams == U_BI_RGB){
           if(colortype != 1 && colortype != 4 && colortype != 8 &&
              colortype != 16 && colortype != 24 && colortype != 32)return(0);
           uint64_t stride = (((uint64_t)width * colortype + 31) / 32) * 4;
           if(!stride || height > cbBits / stride)return(0);
       }
   }
   return(1);
}
...
int U_EMREOF_safe(const char *record){
   ...
   const char *blimit = record + pEmr->emr.nSize;
   uint64_t cbPalEntries=pEmr->cbPalEntries;
   if(cbPalEntries){
      if(IS_MEM_UNSAFE(record, (uint64_t)pEmr->offPalEntries + 2*2, blimit))return(0);// 2 16 bit values in U_LOGPALLETE
   }
   uint64_t off = sizeof(U_EMREOF) + 4 * cbPalEntries;
...
int U_EMRCOMMENT_safe(const char *record){
   ...
   uint64_t cbData = pEmr->cbData;
   const char *blimit =record + pEmr->emr.nSize;
   if(IS_MEM_UNSAFE(record, cbData + U_SIZE_EMRCOMMENT, blimit))return(0);
   return(1);
} 
...
int U_EMRGRADIENTFILL_safe(const char *record){
   ...
   uint64_t nTriVert = pEmr->nTriVert;
   uint64_t nGradObj = pEmr->nGradObj;
