...
void GERBVIEW_FRAME::Liste_D_Codes()
{
    ...
    wxString        Line;
    wxArrayString   list;
    double          scale = GetUserUnits() == EDA_UNITS_T::INCHES ? IU_PER_MILS * 1000 : IU_PER_MM;
    ...
    for( int layer = 0; layer < (int)ImagesMaxCount(); ++layer )
    {
        ...
        const char* units = GetUserUnits() == EDA_UNITS_T::INCHES ? "\"" : "mm";
...
void GERBVIEW_FRAME::DisplayGridMsg()
{
    ...
    int icurr = screen->BuildGridsChoiceList( gridsList, m_userUnits != EDA_UNITS_T::INCHES );
...
void GERBVIEW_FRAME::updateGridSelectBox()
{
    ...
    wxArrayString gridsList;
    int           icurr =
            GetScreen()->BuildGridsChoiceList( gridsList, GetUserUnits() != EDA_UNITS_T::INCHES );
