...
DIALOG_UPDATE_FIELDS_BASE::DIALOG_UPDATE_FIELDS_BASE( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : DIALOG_SHIM( parent, id, title, pos, size, style )
{
	...
	m_removeExtraBox->SetToolTip( _("Removes fields that do not occur in the original library symbol") );
	...
	m_resetEmpty->SetToolTip( _("Clear existing entries if the correspondig library field is empty") );
	...
	m_mainSizer->Add( m_sdbSizer, 0, wxBOTTOM|wxEXPAND|wxLEFT|wxRIGHT|wxTOP, 5 );
