...
BOOST_FIXTURE_TEST_CASE( DRCNetChainTuningProfiles, DRC_REGRESSION_TEST_FIXTURE )
{
    std::vector<std::pair<wxString, int>> tests = {
        { "drc_net_chain_tuning_profiles/test_drc_net_chain_tuning_profiles", 1 }
    };
    ...
    for( const std::pair<wxString, int>& test : tests )
    {
        ...
        bds.m_DRCSeverities[DRCE_NET_CHAIN_TUNING_PROFILES] = SEVERITY::RPT_SEVERITY_ERROR;
        ...
        if( violations.size() == test.second )
        {
            ...
            BOOST_TEST_MESSAGE( wxString::Format( "DRC net chain tuning profiles: %s, passed", test.first ) );
        }
        else
        {
            ...
            BOOST_ERROR(
                    wxString::Format( "DRC net chain tuning profiles: %s, failed (violations found %d expected %d)",
