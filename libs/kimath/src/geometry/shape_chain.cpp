                         ...
                         SHAPE_CHAIN& aPost ) const
{
    assert( false );
        ...
        void SHAPE_CHAIN::Simplify( int aTolerance  ){ assert(false); }

        int SHAPE_CHAIN::Find( const VECTOR2I& aP, int aThreshold  ) const{ return 0; }

        bool SHAPE_CHAIN::Collide( const VECTOR2I& aP, int aClearance , int* aActual ,
                            VECTOR2I* aLocation  ) const { assert(false);return false; }
        bool SHAPE_CHAIN::Collide( const SEG& aSeg, int aClearance , int* aActual ,
                            VECTOR2I* aLocation  ) const { assert(false);return false; }
        ...
        {
            assert(false); return SHAPE_LINE_CHAIN();
        }
